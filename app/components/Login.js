...
export default class Login extends React.Component {
    ...
    firebase
      ...
      .then(() => {
        ...
        this.setState({
          error: '',
          loading: false,
          message: 'Welcome to the Coffee App',
        });
        this.setUserToken();
        Actions.homeView();
      })
  ...
  renderLoaderButtonText() {
    ...
    return <Text style={styleSheet.buttonText}>Log In</Text>;
  }

  generateErrorMessage() {
    if (this.state.message) {
      if (this.state.message.length > 0) {
        return <StatusMessage>{this.state.message}</StatusMessage>;
      }
      return '';
    }
  }
    ...
    return (
      <View style={styleSheet.loginView}>
        ...
        <TextInput
          ...
          secureTextEntry
          onChangeText={userNamePassword =>
            this.setState({ password: userNamePassword })
          }
        />
        <TouchableHighlight
          onPress={() => {
        ...
        >
          <View style={styleSheet.button}>{this.renderLoaderButtonText()}</View>
        </TouchableHighlight>
        <Button
          title="Create new user"
          color={colors.primary}
          onPress={() => {
            this.onCreateUserPress();
          }}
        />
        <View>{this.generateErrorMessage()}</View>
        <Logo />
      </View>
    );
  }
